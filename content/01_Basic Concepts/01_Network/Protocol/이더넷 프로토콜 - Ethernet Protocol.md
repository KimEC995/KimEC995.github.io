...
[[00_ What Is Internet|인터넷]]과 매우 유사한 이름이지만 둘은 엄밀히 다른 개념이다. 현재까지도 [[00_ What is Network|네트워크]] 환경에서 절대 다수를 차지하는 프로토콜로 유선 인터넷을 사용한다면 이더넷을 쓰는 환경이라고 생각하면 된다.

주로 [[LAN - 근거리 통신망|LAN]]에서 사용되지만, 기술의 발전으로 [[WAN - 광역 통신망|WAN]]에서도 확장 되어 사용하기도 한다. 아래는 LAN 기준으로 작성한다.

[[OSI 7 Layer & TCP,IP 4 Layer#데이터 링크 계층|데이터 링크 계층(2층)]] 뿐만 아니라 [[OSI 7 Layer & TCP,IP 4 Layer#물리 계층|물리 계층(1층)]]도 포함하는 기술 중 하나로, 데이터를 [[프레임 - Frame|프레임]] 단위로 전송하는 [[00_프로토콜 - Protocol|프로토콜]]이다.
