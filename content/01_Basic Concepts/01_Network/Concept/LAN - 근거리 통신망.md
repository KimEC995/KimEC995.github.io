...
중요한 기준은 [[IP - Internet Protocol|IP]]주소와 [[서브넷 - Subnet#서브넷 마스크|서브넷 마스크]]이다. **같은 서브넷 마스크와 네트워크 주소를 공유하는 장비들은 동일한 [[IP브로드캐스트 - Broadcast|브로드캐스트]] 도메인**에 속하며, 이 범위를 하나의 LAN이라고 볼 수 있다. 따라서 네트워크의 구성 방식에 따라 라우터가 여러 대 있어도 동일한 서브넷 안에 있다면 하나의 LAN으로 간주할 수 있다.
