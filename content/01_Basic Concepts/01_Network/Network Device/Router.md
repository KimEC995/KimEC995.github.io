...
**라우터는 [[OSI 7 Layer & TCP,IP 4 Layer|L3 계층]] 장비이기 때문에 [[IP - Internet Protocol#비신뢰성|IP의 비신뢰성 특징]]을 지닌다**. 이말은 최적 경로로 데이터 [[패킷]]을 전송하고 그 후에는 응답에 신경쓰지 않는다. 따라서 잘 도착 했는지, 못 도착 했는지를 구분하지 않는다.
